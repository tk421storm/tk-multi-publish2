...
class SummaryOverlay(QtGui.QWidget):
	...
	def show_success(self, deliveryID):
		"""
		...
		self.ui.info.clicked.disconnect(self.info_clicked.emit)
		self.deliveryID=deliveryID
		self.ui.info.setText("To show delivery, <b><u>click here</u></b>.")
	...
	def show_delivery(self):
		"""
		open a webpage to the created delivery
		"""
		
		projectID=self._bundle.sgtk.pipeline_configuration._project_id
		if self.deliveryID:
			url=self._bundle.sgtk.shotgun_url+"/detail/Delivery/"+str(self.deliveryID)
		else:
			url=self._bundle.sgtk.shotgun_url+"/page/project_overview?project_id="+str(projectID)
