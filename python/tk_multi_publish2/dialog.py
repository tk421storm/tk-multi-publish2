...
from tank_vendor import six #@UnresolvedImport
...
class AppDialog(QtGui.QWidget):
    ...
    def setupElementsIngest(self, parent):
        ...
        self.elementTypes=self._bundle.sgtk.shotgun.schema_field_read('Element')['sg_element_type']['properties']['valid_values']['value']
        
        #attempt to parse the yaml and relate it to shotgun-derived field values
        self.elementsConfigFile=yamlConfigFile(self._bundle.sgtk.pipeline_configuration.get_config_location())
        
        if not self.elementsConfigFile:
            errors=['Unable to locate per-project Elements ingest config file']
        else:
            errors=[]
        
        #parse the config from the yaml file with a function
        errors+=self.parseConfig(parseYAML(self.elementsConfigFile))
        
        if len(errors):
            errorMessage=QtGui.QMessageBox(parent=parent)
        ...
        #make sure we have a definition for each element type, if we don't, warn the user
        missingElementTypes=[]
        for elementType in self.elementTypes:
            if elementType not in self.elementExtensions:
                missingElementTypes.append(elementType)
                
        #we know what to do with "Other", even though its not defined per-project, so remove that from the list
        if "Other" in missingElementTypes:
            missingElementTypes.pop(missingElementTypes.index('Other'))
                
        if len(missingElementTypes):
            errorMessage=QtGui.QMessageBox(parent=parent)
            errorMessage.setIcon(QtGui.QMessageBox.Warning)
            errorMessage.setText('The following Element types are defined in shotgun, but not in configuration yaml:\n\n'+'\n'.join(missingElementTypes)+'\n\nThose types will not be avilable to ingest.')
            errorMessage.setStandardButtons(QtGui.QMessageBox.Ok)
            errorMessage.exec_()
                
        #now we'll filter out the element types we dont have a definition for from our lists
        for elementType in missingElementTypes:
            self.elementTypes.pop(self.elementTypes.index(elementType))
            
        removeFromElementExtensions=[]
        for elementType in self.elementExtensions:
            if elementType not in self.elementTypes:
        ...
        for deliveryType in self.deliveryElementLocations:
            removeFromTypeDict=[]
            dicter=self.deliveryElementLocations[deliveryType]
            for elementType in dicter:
                if elementType not in self.elementTypes:
                    removeFromTypeDict.append(elementType)
        ...
        for deliveryType in self.deliveryElementLocations:
            elementTypesDict=self.deliveryElementLocations[deliveryType]
            for elementType in elementTypesDict:
                dicter=elementTypesDict[elementType]
                for keyName in dicter:
                    if keyName not in self.elementExtensionKeys:
                        if keyName not in undefinedExtensions:
                            undefinedExtensions.append(keyName)
                    
                    #also verify that the templates listed for each type are defined in the templates
                    outputTuple=dicter[keyName]
                    for templateString in outputTuple:
                        if not templateString=="None":
                            if templateString not in self._bundle.sgtk.templates:
                                if templateString not in undefinedTemplates:
                                    undefinedTemplates.append(templateString)
                        else:
                            #here we'll also make sure our string "None"s are actual Nones
                            outputTuple[outputTuple.index('None')]=None
                    
        if len(undefinedExtensions):
            errorMessage=QtGui.QMessageBox(parent=parent)
            errorMessage.setIcon(QtGui.QMessageBox.Critical)
            errorMessage.setText('The following file types do not have corresponding file extension lists defined in yaml:\n\n'+'\n'.join(undefinedExtensions)+'\n\nAborting.')
            errorMessage.setStandardButtons(QtGui.QMessageBox.Ok)
            errorMessage.exec_()
        ...
        if len(undefinedTemplates):
            errorMessage=QtGui.QMessageBox(parent=parent)
            errorMessage.setIcon(QtGui.QMessageBox.Critical)
            errorMessage.setText('The following shotgun templates were defined in Elements\' ingest.yaml but are not defined in global templates.yaml:\n\n'+'\n'.join(undefinedTemplates)+'\n\nAborting.')
            errorMessage.setStandardButtons(QtGui.QMessageBox.Ok)
            errorMessage.exec_()
        ...
        print("phosphene elements ingest loading with definitions: ")
        print("   element types:")
    ...
    def do_publish(self):
        ...
        try:
            ...
            if self._bundle.get_setting("validate_on_publish"):
                ...
                if self.do_validate(is_standalone=False) > 0:
                    self._progress_handler.logger.error(
                        "Validation errors detected. " "Not proceeding with Ingest."
                    )
                    self.ui.button_container.show()
                    return
        ...
        if num_issues>0 or num_finalize_issues>0:
            self._progress_handler.logger.error(
                "Element Ingest Failed! For details, click here."
            )
    ...
    def parseConfig(self, configDict):
        '''passed a configuration dictionary (read from per-project yaml file), attempt to parse it and store variables on self.
        
           also verify we have a definition for each discovered delivery type and element type we found - and react accordingly.
           
           return a list of warnings to prompt the user - this will also prevent the tool from loading!
        '''
        
        warnings=[]
        
        #first remove the two required definition dictionaries from the config dict
        if 'deliveryElementLocations' in configDict:
            self.deliveryElementLocations=copy(configDict['deliveryElementLocations'])
            del configDict['deliveryElementLocations']
        else:
